...
/**
 * Domain policy for completion-derived message bookkeeping. Both the run and
 * regenerate use cases need: (a) persist an assistant message after the model
 * answers, (b) drop stale assistant turns before regenerating, (c) build the
 * model's context window from recent history. This policy hosts those pieces
 * so neither use case duplicates them and neither use case has to import the
 * other.
 *
 * Lives in the chat domain — it touches only the message repository interface
 * and a Logger primitive, no controllers, no strategies.
 */
